...
static void AddAssumptionsFromCallSite(CallBase &CB, InlineFunctionInfo &IFI) {
  ...
  for (Argument &Arg : CalledFunc->args()) {
    ...
    if (CB.getAttributes().hasParamAttr(ArgNo, Attribute::NonNull) && CB.getAttributes().hasParamAttr(ArgNo, Attribute::NoUndef) &&
      !isKnownNonZero(ArgVal, DL, 0, AC)) {
...
llvm::InlineResult llvm::InlineFunction(CallBase &CB, InlineFunctionInfo &IFI,
  ...
  { // Scope to destroy VMap after cloning.
    ...
    AddAssumptionsFromCallSite(CB, IFI);
